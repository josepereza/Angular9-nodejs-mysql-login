...
router.post('/register', authGuard.Authorize,async function(req, res, next) {
    console.log('user bueno: ', req.user);

    if(admin==true) {
        var cliente = req.body;
    console.log(cliente.Usuario)
    db.then(conn=>{
        conn.query("insert into users(email,password)values(?,?)",[cliente.Usuario, cliente.Password])
        .then(x=>{
            console.log(x)
           console.log(cliente)
          return res.send(x)
...
router.post('/login', async function(req, res, next) {
    var user = req.body;
console.log('este el el login especial jajaja ' + user);
    //Verifica el usuario
    if (!user)
                    ...
                    try {
                        ...
                        jwt.sign({ cusu: user.Usuario }, '123456', signOptions,
                            function(err, token) {
                                ...
                                console.log('token de bueno: ', token);
